...
def eval_cand(point, voisins, solution, inst, demand, capacity, fe, mode):
    (r1, r2), v = route.another_route(point, voisins,
                                      solution, demand, inst, capacity, fe, "EC", mode)
    if v < 0:
        return const.Error
...
def rd_cand(route, np, voisins, solution, inst, demand, capacity, fe, mode):
    parcours = utile.permut([i for i in range(len(route))])
    best = 0
    for i in parcours:
        p = route[i]
        if p != np:
            cp = utile.rd_point((route[i-1], p), solution, inst)
            cand = eval_cand(cp, voisins, solution, inst,
                             demand, capacity, fe, mode)
            if cand[0] > best:
                if mode == "RD":
                    return cand
                elif mode == "DE":
...
def ejection_chain(point, voisins, solution, inst, demand, capacity, fe, mode):
    ...
    for k in range(const.relocation-1):
        ...
        s, I, R = rd_cand(curr_route, relocated_cust,
                          voisins, solution, inst, demand, capacity, fe, mode)
